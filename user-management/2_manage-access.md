...
As you are planning and implementing your user and access control permissions for your Azure Databricks instances, you can manage workspaces and access control for instance objects.  These objects include the following: workspaces, clusters, jobs, tables and use of token-based authentication for authorized users.

## Manage Workspace Storage

Admin users manage workspace storage on the 'Workspace Storage; tab in the Admin Console page.

### Purge Workspace Information
...
Click the Purge button next to 'permanently purge workspace storage', then click 'Yes, purge' to confirm. 
...
IMPORTANT: Once purged, workspace objects, revision or cluster logs are not recoverable.

## Manage Access Control  
The following section describes how to set up access control and enable token-based authentication for your Azure Databricks instance and includes the following topics:
...
You can assign five permission levels to notebooks and folders: No Permissions, Read, Run, Edit, and Manage. The tables list the abilities for each permission.  
 ...
 ![workspace permissions](media/workspace-perms.png) 
...
2. Grant permissions. 
    - All users in your account belong to the group all users. Administrators belong to the group admins, which has Manage permissions on all items
...
#### Library and jobs access control
Note: All users can view libraries. To control who can attach libraries to clusters, see Cluster Access Control.  To control who can run jobs and see the results of job runs, see Jobs Access Control.
...
![cluster permissions](media/cluster-perms.png)
...
![job permissions](media/job-perms.png)
Notes:
...
For information about how to set permissions on a data object once table access control is enabled, see [Set Permissions on a Data Object](https://docs.azuredatabricks.net/administration-guide/admin-settings/table-acls/object-permissions.html).
...
Note: Starting with Databricks Platform version 2.68, all access controls are enabled by default for new accounts in the Premium SKU. For accounts that were opened before version 2.68, an administrator must enable table access control using the steps listed in this section.
...
![enable table access](media/enable-table-access.png)
...
Next you can Set Permissions on a Data Object.
...
Note: Azure Databricks does not support an explicit DENY command for objects.
...
![sql permissions](media/sql-perms.png)
...
For more information about using token-based authentication, see [Authentication](https://docs.azuredatabricks.net/api/latest/authentication.html).
